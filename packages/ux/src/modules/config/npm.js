...
export function processNpmBuildConfig({ report, userConfig }) {
  let { cjs, esModules, umd, ...unexpectedConfig } = userConfig.npm
  ...
  if (unexpectedProps.length > 0) {
    report.error(
      ...
      `Unexpected prop${s(unexpectedProps.length)} in ${chalk.cyan(
        'babel'
      )} config - ` +
  ...
  if ('cjs' in userConfig.npm) {
    if (typeOf(cjs) !== 'boolean') {
      report.error('npm.cjs', cjs, `Must be ${chalk.cyan('Boolean')}`)
    }
  }
    ...
    if (umd === false) {
      ...
      report.error(
        ...
        `Must be a ${chalk.cyan('String')} (for ${chalk.cyan('global')} ` +
          `config only)}, an ${chalk.cyan(
            'Object'
          )} (for any UMD build options) ` +
          `or ${chalk.cyan('false')} (to explicitly disable the UMD build)`
      )
    } else {
      let { entry, global: umdGlobal, externals, ...unexpectedConfig } = umd
