...
export default class UserConfigReport {
  ...
  getReport() {
    ...
    report.push(
      chalk.underline(`nwb config report for ${this.getConfigSource()}`)
    )
    report.push('')
    ...
    this.errors.forEach(({ path, value, message }) => {
      report.push(
        `${chalk.red(`${figures.cross} ${path}`)} ${chalk.cyan(
          '='
        )} ${util.inspect(value)}`
      )
      report.push(`  ${message}`)
    ...
    if (this.deprecations.length) {
      let count =
        this.deprecations.length > 1 ? `${this.deprecations.length} ` : ''
      report.push(
        chalk.yellow.underline(
          `${count}Deprecation Warning${s(this.deprecations.length)}`
        )
      )
      report.push('')
    }
