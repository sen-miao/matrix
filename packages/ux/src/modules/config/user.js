...
export function getUserConfig(args = {}, options = {}) {
  ...
  if (configFileExists) {
    try {
      ...
      throw new Error(
        `Couldn't import the config file at ${configPath}: ${e.message}\n${
          e.stack
        }`
      )
    }
...
export function getProjectType(args = {}) {
  ...
  if (!configFileExists) {
    throw new Error(
      `Couldn't find a config file at ${configPath} to determine project type.`
    )
  }
  ...
  if (!PROJECT_TYPES.has(userConfig.type)) {
    report.error(
      'type',
      userConfig.type,
      `Must be one of: ${joinAnd(Array.from(PROJECT_TYPES), 'or')}`
    )
  }
  if (report.hasErrors()) {
...
export function processUserConfig({
  ...
  let {
    ...
    // karma,
    // npm,
    webpack: _webpack, // eslint-disable-line no-unused-vars
    ...unexpectedConfig
  } = userConfig
