...
const mochaRoot = path.dirname(require.resolve('mocha'))
...
program
  .version(
    JSON.parse(fs.readFileSync(path.join(mochaRoot, 'package.json'), 'utf8'))
      .version
  )
...
requires.forEach(mod => {
  if (mod === '@babel/register') {
    require(mod)({
      extends: path.join(context, '.babelrc'),
      plugins: [
        [
          require.resolve('babel-plugin-module-resolver'),
          {
            alias: {
              react: `${context}/node_modules/react`,
              chai: `${context}/node_modules/chai`,
              sinon: `${context}/node_modules/sinon`,
              enzyme: `${context}/node_modules/enzyme`
            }
          }
        ]
      ]
